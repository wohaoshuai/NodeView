...
//  NodeView
//
//  Created by Kelong Wu on 2/28/17.
...
class Control: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        save("1", view)
        cache["root"] = Node()
        cache["root"]!.rule["init"] = ["perform", "29f55fe0-17af-45a0-9ee3-42b0bf4ea268", "1", "2"]
        env.append("root")
        eval(rule: cache["root"]!.rule["init"]!)
        print(load("2"))
    }
    ...
    func eval(rule: [String]){
        ...
        case "perform":
            ...
            var a3 = [Any]()
            if rule.count > 4{
                for i in 4...(rule.count - 1){
                    a3.append(load(rule[i]))
                }
            }
            initer(a1, a2, a3, p1)
        case "list":
            if rule.count > 1{
                for i in 1...(rule.count - 1){
                    let new_rule = load_rule(env.last!, rule[i])
                    eval(rule: new_rule)
                }
            }
        case "jump":
            let new_env = rule[1]
            let new_rule = load_rule(rule[1], rule[2])
            env.append(new_env)
            eval(rule: new_rule)
            env.removeLast()
        default:
    ...
    func initer(_ a1: String, _ a2: Any, _ a3: [Any], _ p1: String){
        /* MARK: - UIView*/
        ...
        if a1 == "c481c21b-cd5c-4ffe-916e-dfe588ef25a9" {
            var v1: UIUserInterfaceLayoutDirection
            if #available(iOS 10.0, *) {
                v1 = UIView.userInterfaceLayoutDirection(for: a3[0] as! UISemanticContentAttribute,relativeTo: a3[1] as! UIUserInterfaceLayoutDirection)
            } else {
    ...
    func save(_ p1: String, _ v1: Any){
        raw[p1] = v1
    }
    
    func load(_ a1: String)->Any{
        return raw[a1]!
    }
    
    func save_rule(_ p1: String, _ p2: String, _ v1: [String]){
        cache[p1]!.rule[p2]! = v1
    }
    
    func load_rule(_ p1: String, _ p2: String)->[String]{
        return cache[p1]!.rule[p2]!
    }
}
